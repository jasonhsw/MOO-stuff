...
;;$page_utils.("help_msg") = {"$page_utils:belongs_to(item_number, page_length): Returns the page that item_number belongs to.", "$page_utils:min_item(page, length): Returns the first item that belongs to page.", "$page_utils:max_item(page, page_length[, max-items]): Returns the last item that belongs to page.", "$page_utils:max_pages(max-items, page-length): Returns the number of pages.", "$page_utils:get_item_reversed(item, page, page_length, max-items): Returns the index of item in page when items are in reverse order.", "$page_utils:min_item_reversed(page, page-length, max-items): Returns the first item belonging to page when items are in reverse order.", "$page_utils:max_item_reversed(page, page-length, max-items): Returns the last item belonging to page when items are in reverse order.", "$page_utils:get_item(item, page, page-length): Return the index of the item in page.", "$page_utils:belongs_to_reversed(item_number, page_length, max_items): Returns the page that item_number belongs to, when items are in reverse order.", "", "This object and all related code was originally written by Jason SantaAna-White.", "This  object and other MOO code is available from https://github.com/jasonhsw/MOO-stuff"}
...
"$page_utils:belongs_to(item_number, page_length): Returns the page that item_number belongs to.";
{item, length} = args;
return toint(((tofloat(item) - 1.0) / tofloat(length)) + 1.0);
"Last modified Sat Feb  2 14:55:52 2019 CST by Codesmith Jason Perino (#91@ThetaCore).";
.
...
"$page_utils:min_item_reversed(page, page-length, max-items): Returns the first item belonging to page when items are in reverse order.";
{page, pagelen, max_items} = args;
return max_items - ((page - 1) * pagelen);
"Last modified Sat Feb  2 15:02:37 2019 CST by Codesmith Jason Perino (#91@ThetaCore).";
.
@verb $page_utils:"max_item_reversed" this none this
@program $page_utils:max_item_reversed
"$page_utils:max_item_reversed(page, page-length, max-items): Returns the last item belonging to page when items are in reverse order.";
{page, pagelen, max_items} = args;
...
return ret;
"Last modified Sat Feb  2 15:03:19 2019 CST by Codesmith Jason Perino (#91@ThetaCore).";
.
